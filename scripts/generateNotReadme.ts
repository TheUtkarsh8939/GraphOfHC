...
const main = async () => {
	...
	for (let i = 0; i < deltaEntries.length; i++) {
		...
		if (countWords(description) < 10 && isValidGithubRepoUrl(entry.code_url)) {
        ...
        console.log(`Processed ${i + 1} of ${deltaEntries.length} entries.`);
		output.push({ id: entry.id, description, readme });
	}
